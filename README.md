...
## Code auf GitHub sichtbar machen
Falls im GitHub-Repository nichts angezeigt wird, ist meist kein Remote gesetzt oder es wird noch nicht auf den Hauptbranch `main` gepusht. Vorgehen:

```bash
# einmalig Remote hinzufügen
git remote add origin <git@github.com:dein-account/quicstar.git>

# sicherstellen, dass der lokale Branch main heißt
git branch -m main
...
Falls bereits ein Branch wie `work` existiert, der den aktuellen Stand enthält, wechsle und benenne ihn vor dem Push um:

```bash
git checkout work    # nur falls der Code dort liegt
git branch -m main   # branch in main umbenennen
git push -u origin main
```

So erscheint dein aktueller Stand auf GitHub im Branch `main`. Kontrolliere bei Bedarf mit `git remote -v` und `git status`, ob Remote und Branch passen.
